...
export default function ThemeSelector() {
  ...
  return (
    ...
    <div class="flex items-center gap-0.5 md:gap-1 text-secondary-contrast dark:text-primary-contrast-dark">
      ...
      <IconSun class="w-1 h-1 md:w-1.5 md:h-1.5" aria-hidden="true"/>

      <label for="colorThemeSwitchToggle"
             onClick={toggleTheme}
             aria-label="Color theme switch"
             role="switch"
             title={isThemeDark() ? 'Dark color theme selected':'Light color theme selected'}
      ...
      <IconMoon class="w-1 h-1 md:w-1.5 md:h-1.5" aria-hidden="true"/>
